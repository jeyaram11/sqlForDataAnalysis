--Project 4
--create one varaible as set value to jan1 
--create table consisting of a list of all claim numbers with date for most recent published for each claim
--create a table with a list of each claimid with its associated assigned date 
use Insurance;
...
SELECT  
	...
	CASE WHEN DaysSinceAdjustedAssigned > 14 AND DaysSinceLastPublished > 90 THEN 0 
		WHEN 91 - DaysSinceLastPublished >= 15 -DaysSinceAdjustedAssigned
		 THEN 91 - DaysSinceLastPublished 
		 ELSE 15- DaysSinceAdjustedAssigned
		 END AS daysToComplete,
    CASE WHEN DaysSinceAdjustedAssigned <= 14 OR DaysSinceLastPublished <= 90 THEN 0
		WHEN DaysSinceLastPublished - 90 <= DaysSinceAdjustedAssigned - 14 THEN DaysSinceLastPublished - 90
