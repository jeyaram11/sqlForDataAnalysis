...
SELECT 
	*,
	case when brand1 > brand2 THEN CONCAT(brand1, brand2,year) else CONCAT (brand2,brand1,year) end as key_id
FROM brands
),
Sub2 as
(
 SELECT 
	*,
	ROW_NUMBER() Over(Partition BY key_id Order by key_id) as ranking
 FROM sub
)
select 
	* 
